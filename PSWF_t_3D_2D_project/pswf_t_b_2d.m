function images = pswf_t_b_2d( coeffs, PSWF_Nn_p, realFlag )
...
% PSWF_m = bsxfun(@times,PSWF_Nn_p.samples,(PSWF_Nn_p.beta/2)*PSWF_Nn_p.alpha_Nn.'); 
PSWF_m = PSWF_Nn_p.samples; 
if (realFlag == 1)
    images_c = PSWF_m(:,PSWF_Nn_p.ang_freq==0)*coeffs(PSWF_Nn_p.ang_freq==0,:) + 2*real(PSWF_m(:,PSWF_Nn_p.ang_freq~=0)*coeffs(PSWF_Nn_p.ang_freq~=0,:));
else
    images_c = ([conj(PSWF_m(:,PSWF_Nn_p.ang_freq>0)) PSWF_m]*coeffs);
end
